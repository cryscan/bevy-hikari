...
@group(3) @binding(2)
var previous_short_texture: texture_2d<f32>;
@group(3) @binding(3)
var short_output_texture: texture_storage_2d<rgba16float, read_write>;
...
fn sample_previous_short_texture(uv: vec2<f32>) -> vec3<f32> {
    let c = textureSampleLevel(previous_short_texture, linear_sampler, uv, 0.0).rgb;
    return clamp(c, vec3<f32>(0.0), vec3<f32>(1.0));
}

fn sample_short_texture(uv: vec2<f32>) -> vec3<f32> {
    let coords = vec2<i32>(uv * vec2<f32>(textureDimensions(short_output_texture)));
    let c = textureLoad(short_output_texture, coords).rgb;
...
fn taa_short(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    previous_color += sample_previous_short_texture(vec2<f32>(texel_position_12.x, texel_position_0.y)) * w12.x * w0.y;
    previous_color += sample_previous_short_texture(vec2<f32>(texel_position_0.x, texel_position_12.y)) * w0.x * w12.y;
    previous_color += sample_previous_short_texture(vec2<f32>(texel_position_12.x, texel_position_12.y)) * w12.x * w12.y;
